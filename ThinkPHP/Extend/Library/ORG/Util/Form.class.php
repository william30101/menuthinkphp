...
class Form {
	...
	public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1,$alowuploadexts = '',$height = 200,$disabled_page = 0, $allowuploadnum = '10') {
		...
		if(!defined('EDITOR_INIT')) {
			$str = '<script type="text/javascript" src="'.__JS_PATH__.'ckeditor/ckeditor.js"></script>';
...
//         if($allowupload) $str .= "filebrowserUploadUrl : '".APP_PATH."index.php?m=attachment&c=attachments&a=upload&module=".$module."&catid=".$catid."&dosubmit=1',\r\n";
		...
		if(!defined('IMAGES_INIT')) {
			$ext_str .= '<script type="text/javascript" src="'.__JS_PATH__.'swfupload/swf2ckeditor.js"></script>';
	...
	public static function images($name, $id = '', $value = '', $moudle='', $catid='', $size = 50, $class = '', $ext = '', $alowexts = '',$thumb_setting = array(),$watermark_setting = 0 ) {
		...
		if(!defined('IMAGES_INIT')) {
			$str = '<script type="text/javascript" src="'.__JS_PATH__.'swfupload/swf2ckeditor.js"></script>';
			define('IMAGES_INIT', 1);
		}
	...
	public static function date($name, $value="", $isdatetime = 0, $loadjs = 0, $showweek = true){
		...
		if($loadjs || !defined('CALENDAR_INIT')) {
			...
			$str .= '<link rel="stylesheet" type="text/css" href="'.__JS_PATH__.'/calendar/calendar_new/css/jscal2.css"/>
			<link rel="stylesheet" type="text/css" href="'.__JS_PATH__.'/calendar/calendar_new/css/border-radius.css"/>
			<script type="text/javascript" src="'.__JS_PATH__.'/calendar/calendar_new/js/jscal2.js"></script>
			<script type="text/javascript" src="'.__JS_PATH__.'/calendar/calendar_new/js/en.js"></script>';
		}
	...
	/*public static function date($name, $value = '', $isdatetime = 0, $loadjs = 0, $showweek = 'true') {
		...
		if($loadjs || !defined('CALENDAR_INIT')) {
			define('CALENDAR_INIT', 1);
			$str .= '<link rel="stylesheet" type="text/css" href="'.__JS_PATH__.'calendar/jscal2.css"/>
			<link rel="stylesheet" type="text/css" href="'.__JS_PATH__.'calendar/border-radius.css"/>
			<link rel="stylesheet" type="text/css" href="'.__JS_PATH__.'calendar/win2k.css"/>
			<script type="text/javascript" src="'.__JS_PATH__.'calendar/calendar.js"></script>
			<script type="text/javascript" src="'.__JS_PATH__.'calendar/lang/en.js"></script>';
		}
	 ...
	 * 下拉选择框
	 */
	public static function select($array = array(), $id = 0, $str = '', $default_option = '',$suffix='') {
		$string = '<select '.$str.'>';
		...
		foreach($array as $key=>$value) {
			...
			$string .= '<option value="'.$key.'" '.$selected.'>'.$value.$suffix.'</option>';
		}
		$string .= '</select>';
		return $string;
	}
	 ...
	 * 仿select下拉框设计（通用）
	 * author Tony
	 * @param unknown $array	数据集合
	 * @param number $id		已经选中的数据key
	 * @param string $name		input的name名和id名
	 * @param string $width		下拉框的宽度，如：'90'为'width:90px;'		
	 * @param string $default	默认显示的，如：'请选择'
	 * @param string $must		是否必填（必填的话，会提示必填）
	 * @param string $suffix	后缀，如:厘米、年、月、日
	 * @return boolean|string	返回值
	 */
	public static function T_sel($array = array(), $id = 0, $name = '', $width='90', $default = '',$must='', $suffix='',$is_search=0) {
		$string   = '<div class="sel T_sel" style=" float: left;width:'.$width.'px;">';
		$string .= '<input class="hideInput" must="'.$must.'" type="hidden" name="'.$name.'" id="'.$name.'" value="'.$id.'">';
		if(!is_array($array) || count($array)== 0) return false;
		if($id){
			$idVal = $array[$id];
			$string .= '<p class="selcon"><span style="height:23px;overflow:hidden;width:'.($width-40).'px;">'.$idVal.'</span><i></i></p>';
		}elseif($default){
			$string .= '<p class="selcon"><span style="height:23px;overflow:hidden;width:'.($width-40).'px;">'.$default.'</span><i></i></p>';
		}
		$ids = array();
		if(isset($id)) $ids = explode(',', $id);
		if($array){
			$gdt = '';
			if(count($array) > 7) $gdt = 'gdt';
			$string .= ' <div class="down '.$gdt.'" style=" display:none;width:'.$width.'px;">';
		}
//         if($default!='')
//         {
//             $string .= '<p>'.$default.'</p>';
//         }
		...
		foreach($array as $key=>$value) {
			$hover = in_array($key, $ids) ? 'class="hover"' : '';
			$pWidth = $width-'18';
			$string .= '<p style="width:'.$pWidth.'px;" data="'.$key.'" '.$hover.'>'.$value.'</p>';
		}
		if($array){
			$string .= '</div>';
		}
		$string .= '</div>';
        //<span style="float: left; padding:5px 5px 0 5px;">'.$suffix.'</span><span style="margin-left:20px;" id="'.$name.'_info"></span>';
		if($is_search==0)
        {
        	if($suffix){
            	$string .= '<span style="float: left; padding:5px 5px 0 5px;">'.$suffix.'</span>';
        	}
            $string .= '<span id="'.$name.'_info"></span>';
        }
        return $string;
	}
	 ...
	 * @return string
	 */
	public static function T_ymd($year, $month, $day, $yearId, $monthId, $dayId, $width='148', $default = '请选择',$type=0) {
		$string   = '<div id="T_ymd_birthday" class="sel ymd" showtype='.$type.' style=" float: left;width:'.$width.'px;">';
		if($year && $month && $day){
			if($month <= 9){
				$month = '0'.$month;
			}
			...
			$string .= '<p class="selcon"><span name="spanDis" style="height:23px;overflow:hidden;width:'.($width-40).'px;">'.$year.'-'.$month.'-'.$day.'</span><i></i></p>';
		}elseif($default){
			$string .= '<p class="selcon"><span name="spanDis" style="height:23px;overflow:hidden;width:'.($width-40).'px;">'.$default.'</span><i></i></p>';
		}
		
		$string .= '<input id="'.$yearId.'" type="hidden" autocomplete="off" name="'.$yearId.'" value="'.$year.'">';
		$string .= '<input id="'.$monthId.'" type="hidden" autocomplete="off" name="'.$monthId.'" value="'.$month.'">';
		$string .= '<input id="'.$dayId.'" type="hidden" autocomplete="off" name="'.$dayId.'" value="'.$day.'">';
		//从90后到30后
		$yearStr =  Form::yearXXToStr(9,$type);
		$yearStr .= Form::yearXXToStr(8,$type);
		$yearStr .= Form::yearXXToStr(7,$type);
		$yearStr .= Form::yearXXToStr(6,$type);
		$yearStr .= Form::yearXXToStr(5,$type);
		$yearStr .= Form::yearXXToStr(4,$type);
		$yearStr .= Form::yearXXToStr(3,$type);
		$dynamicWidth = '';
		if($type == 1){
			$string .= '<div id="'.$yearId.'_val" class="reg-data yStyleClass" style="width:300px;display: none;">'.$yearStr.'</div>';
			$string .= '<div id="'.$monthId.'_val" class="reg-moon mStyleClass" style="width:300px;display: none;"></div>';
			$string .= '<div id="'.$dayId.'_val" class="reg-day dStyleClass" style="width:300px;display: none;"></div>';
		}else{
			$string .= '<div id="'.$yearId.'_val" class="reg-data yStyleClass" style="display: none;">'.$yearStr.'</div>';
			$string .= '<div id="'.$monthId.'_val" class="reg-moon mStyleClass" style="display: none;"></div>';
			$string .= '<div id="'.$dayId.'_val" class="reg-day dStyleClass" style="display: none;"></div>';
		}
	 ...
	 * @return string
	 */
	private function yearXXToStr($year,$type=null){
		$string = '';
		if($year){
			if($type == 1){
				$useTop = '<table width="300" cellspacing="0" cellpadding="0" border="0"><tbody><tr class="reg-rbg">';
				$useBtm = '</tr></tbody></table>';
				$yearStr = $useTop.'<td class="w1" style="width: 48px;">19'.$year.'0后：</td>';
				for($i=0;$i<=9;$i++){
					if($year == 9 && $i > 6){
						$yearStr .= '<td style="width: 22px;"></td>';
					}else{
						$yearStr .= '<td class="ymdClick w2" style="width: 22px;"><a data="19'.$year.$i.'" >'.$year.$i.'</a></td>';
					}
				}
			}else{
				$useTop = '<table width="405" cellspacing="0" cellpadding="0" border="0"><tbody><tr class="reg-rbg">';
				$useBtm = '</tr></tbody></table>';
				$yearStr = $useTop.'<td class="w1">'.$year.'0后：</td>';
				for($i=0;$i<=9;$i++){
					if($year == 9 && $i > 6){
						$yearStr .= '<td style="width: 37px;"></td>';
					}else{
						$yearStr .= '<td class="ymdClick w2"><a data="19'.$year.$i.'" >19'.$year.$i.'</a></td>';
					}
				}
			}
			$string = $useTop.$yearStr.$useBtm;
		}
		return $string;
	}
	 ...
	 * @param $width 宽度
	 */
	public static function checkbox($array = array(), $id = '', $str = '', $defaultvalue = '', $width = 0, $field = '',$type='',$br_num=5,$check_code='') {
		$string = '';
		$id = trim($id);
        ...
        if($type=='table')
        {
            $string .='<table width="98%" border="0" align="left" cellpadding="0" cellspacing="0">';
            $br_num_i=1;
        }
		foreach($array as $key=>$value) {
            if($type=='table')
            {
                if($i==1)
                {
                    $string .="<tr><td width='20%'>";
                }
            }
            if(!defined('LOVAL_CHECKBOX_INIT_'.$field)) {
                define('LOVAL_CHECKBOX_INIT_'.$field, 1);
                $local_code = $check_code;
            }
            else
            {
                $local_code = '';
			...
			$string .= '<input type="checkbox" '.$str.' id="'.$field.'_'.$i.'" '.$checked.' value="'.htmlspecialchars($key).'" '.$local_code.'>'.htmlspecialchars($value);
			if($width) $string .= '</label>';

            if($type=='table')
            {
                $string .="</td>";
                if($i%$br_num==0 && $key==(sizeof($array)))
                {
                   $string .="</tr></table>";
                ...
                if($i%$br_num==0 &&  $key!=(sizeof($array)))
                {
                    $string .="</tr><tr><td  width='20%'>";
                }
                elseif($i%$br_num==0 &&  $key==(sizeof($array)))
                {
                ...
                if($key==(sizeof($array)) && $i%$br_num!=0)
                {
                    $string .="</tr></table>";
                }
                else
     ...
     * @param $width 宽度
     */
    public static function checkbox_li($array = array(), $id = '', $str = '', $defaultvalue = '', $width = 0, $field = '',$type='',$br_num=5,$check_code='') {
        $string = '';
        $id = trim($id);
        if($id != '') $id = strpos($id, ',') ? explode(',', $id) : array($id);
        if($defaultvalue) $string .= '<input type="hidden" '.$str.' value="-99">';
	...
	/**
	 *  多级联动并多选组件
	 *  author Tony 2011-06-03
	 *  linkboxs
	 *
	 **/
	public static function select_linkboxs($keyid = 0, $parentid = 0, $name = 'parentid', $id ='', $alt = '', $linkboxsid = 0, $property = '') {
		$tree = pc_base::load_sys_class('tree');
		$result = getcache($keyid,'linkboxs');
		$id = $id ? $id : $name;
		$string = "<select name='$name' id='$id' $property>\n<option value='0'>$alt</option>\n";
		if($result['data']) {
			foreach($result['data'] as $area) {	
				$categorys[$area['linkboxsid']] = array('id'=>$area['linkboxsid'], 'parentid'=>$area['parentid'], 'name'=>$area['name']);	
			}
		}
		$str  = "<option value='\$id' \$selected>\$spacer \$name</option>";

		$tree->init($categorys);
		$string .= $tree->get_tree($parentid, $str, $linkboxsid);
	 ...
	 * 2012.02.17
	 */
	public static function select_linkpc($keyid = 0, $parentid = 0, $name = 'parentid', $id ='', $alt = '', $linkageid = 0, $property = '') {
		$tree = pc_base::load_sys_class('tree');
		$result = getcache($keyid,'linkage');
		$id = $id ? $id : $name;
		$string = "<select name='$name' id='$id' $property>\n<option value='0'>$alt</option>\n";
