...
class Tenpay extends paymentabstract{
	...
	public function __construct($config = array()) {	
		...
		$this->config['gateway_url'] = 'https://www.tenpay.com/cooperate/gateway.do?_input_charset=utf-8';
		$this->config['gateway_method'] = 'POST';
		$this->config['notify_url'] = return_url('tenpay',1);
		$this->config['return_url'] = return_url('tenpay');
        include_once SYSTEM_PATH.APP_PATH.'Common/tenpay.func.php';
	...
	public function getpreparedata() {		
		...
		$prepare_data['seller_email'] = $this->config['tenpay_account'];
		$prepare_data['partner'] = $this->config['tenpay_partner'];
		...
		// 数字签名
		$prepare_data['sign'] = build_mysign($prepare_data,$this->config['tenpay_key'],'MD5');
		return $prepare_data;
	}
    ...
    public function receive() {
    	...
    	if ($receive_data) {
			$verify_result = $this->get_verify('http://notify.tenpay.com/trade/notify_query.do?partner=' . $this->config['tenpay_partner'] . '&notify_id=' . $receive_data['notify_id']);
